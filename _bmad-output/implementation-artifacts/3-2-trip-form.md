...
**AC5:** Given I change any setting value, when the change event fires, then the new value is immediately written to `localStorage` under key `chekov_settings` — no Save button required; `DEFAULT_SETTINGS` and `SETTINGS_KEY = "chekov_settings"` are defined only in `state.js` and never duplicated elsewhere.
...
- [x] Task 4: Update `static/js/app.js` — form & settings interaction
  ...
  - [x] Trip field persistence: save `origin`, `destination`, `range_km`, `waypoint` to localStorage on `change` event (separate from settings — use `localStorage.setItem("chekov_trip", JSON.stringify({...}))`)
  - [x] `initForm()` also loads `chekov_trip` from localStorage to pre-populate trip fields on reload
...
- `chekov_settings` (`SETTINGS_KEY` from `state.js`) — fuel type, tank size, corridor, safety buffer, max alternatives
- `chekov_trip` — trip-specific fields (origin, destination, range_km, waypoint) stored by `app.js`

The `chekov_trip` key is managed in `app.js` directly (it's trip state, not settings). Do NOT put it in `state.js` or use the `saveSettings()` function for it.
