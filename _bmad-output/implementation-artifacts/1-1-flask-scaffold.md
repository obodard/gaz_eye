...
**AC3:** `chekov.py` and `stations.yaml` are preserved entirely unchanged.
...
- [x] Task 3: Create `api/__init__.py` and `api/routes.py` with Blueprint skeleton
  ...
  - [x] `api/routes.py` — Flask Blueprint `bp`, no routes yet, placeholder `GET /` returning 200 "chekov" HTML
...
- `chekov.py` and `stations.yaml` must remain 100% unchanged
...
- Created `api/__init__.py` (empty) and `api/routes.py` with Flask Blueprint `bp`; `GET /` returns `<h1>chekov</h1>` as placeholder
...
### Completion Notes
✅ All ACs satisfied:
- AC1: Flask starts, `GET /` returns 200 with "chekov", warning printed when `GOOGLE_MAPS_API_KEY` unset
- AC2: All required files exist; `app.py` has zero `@app.route` decorators (verified via AST)
- AC3: `chekov.py` and `stations.yaml` unchanged (git diff clean)
