...
- [x] Task 1: Update `api/routes.py` — replace placeholder `serve_index` with Jinja2 render
  - [x] Replace `render_template_string("<h1>chekov</h1>")` with `render_template("index.html", google_maps_api_key=current_app.config["GOOGLE_MAPS_API_KEY"])`
...
- [x] Task 5: Create `static/js/state.js`
  - [x] `const SETTINGS_KEY = "chekov_settings";` — defined once here, never duplicated elsewhere
...
export function initMap() {
    ...
    map = new google.maps.Map(mapEl, {
        ...
        zoom: 7,
        mapId: "chekov_map",  // Required for AdvancedMarkerElement (Story 3.4)
    });
}
