...
**AC4:** Given `chekov.py` exists after the extraction, when it is run directly (`python chekov.py`), then it still works correctly — the module extraction does not break the existing CLI.
...
- [x] Task 2: Ensure `chekov.py` still works after extraction
  - [x] Verified `chekov.py` uses its own local `fetch_stations()` (unchanged)
...
**Extraction Strategy:**
- `api/pricing.py` is a NEW module — it does NOT import from `chekov.py`
- `chekov.py` stays 100% unchanged — it keeps its own copy of `fetch_stations` (which calls `sys.exit()`) and `parse_price_value`
...
- The new `parse_price_value` takes a raw string (NOT a dict/price_entry like in chekov.py) — simpler interface for the API use case
...
### Implementation Plan
- Created `api/pricing.py` as a new independent module (does NOT import from `chekov.py`)
...
- `chekov.py` unchanged — still has its own `fetch_stations` that calls `sys.exit()`
...
- AC4: `chekov.py` unchanged (git diff clean, parses without error)
