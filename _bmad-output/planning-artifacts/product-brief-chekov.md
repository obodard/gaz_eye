...
Chekov is a locally-hosted web application that combines route planning and fuel optimization into a single view:
...
- **Route-first, not station-first.** Unlike GasBuddy or Waze fuel features that show nearby cheap stations, Chekov shows the cheapest station *on your actual driving path*. The question isn't "where's cheap gas near me?" — it's "which route has the cheapest gas?"
...
- **Backend:** Python — reuses the existing pricing pipeline from `chekov.py` (GeoJSON fetch, price parsing, delta calculations)
