...
- `gaz_saver.py` is deleted; `api/pricing.py` is the single Régie Essence integration point
...
- `gaz_saver.py` deleted — `api/pricing.py` is the sole Régie Essence integration
...
Olivier opens Gaz Eye and instead of filling the form, types into the chat panel: *"Montréal to Duhamel, 180 km range."*
...
- The agent defines structured tools corresponding to gaz_eye actions:
...
- `gaz_saver.py` has been deleted; `api/pricing.py` is the canonical Régie Essence client — do not reintroduce the standalone CLI
